...
const { assetsDir, publicDir, dataDir } = require('./paths')
...
const API_RATE_LIMIT_RPM = Number(process.env.API_RATE_LIMIT_RPM || 900)
...
function rateLimit() {
  ...
  return (req, res, next) => {
    ...
    b.count += 1
    buckets.set(key, b)
    res.setHeader('X-RateLimit-Limit', String(API_RATE_LIMIT_RPM))
    res.setHeader('X-RateLimit-Remaining', String(Math.max(0, API_RATE_LIMIT_RPM - b.count)))
    res.setHeader('X-RateLimit-Reset', String(Math.floor(b.resetAt / 1000)))
    if (b.count > API_RATE_LIMIT_RPM) return res.status(429).json({ ok: false, error: 'Rate limit' })
    next()
  }
}
...
function createApiRouter() {
  ...
  router.get('/wa/qr', asyncHandler(async (req, res) => {
    const qr = getLastQr()
  ...
  router.get('/chat/chats', asyncHandler(async (_, res) => {
    const rows = await listChats()
    res.json({ ok: true, chats: rows })
  }))

  router.get('/chat/events', (req, res) => {
    addClient(req, res)
  })

  router.post('/chat/subscribe', asyncHandler(async (req, res) => {
    const sock = getSock()
  ...
  router.get('/chat/download/:id', asyncHandler(async (req, res) => {
  ...
  router.get('/wa/groups', asyncHandler(async (_, res) => {
    const sock = getSock()
    if (!sock) return res.status(503).json({ ok: false, error: 'Bot não conectado' })
  ...
  router.get('/wa/groups/:jid', asyncHandler(async (req, res) => {
    const sock = getSock()
    if (!sock) return res.status(503).json({ ok: false, error: 'Bot não conectado' })
  ...
  router.post('/wa/groups/:jid/extract', asyncHandler(async (req, res) => {
  ...
  router.post('/clients/:id/labels', (req, res) => {
  ...
  router.delete('/clients/:id/labels/:labelId', (req, res) => {
  ...
  router.get('/clients/by-label/:labelId', (req, res) => {
  ...
  router.post('/send/flow', asyncHandler(async (req, res) => {
  ...
  router.post('/send/audio', asyncHandler(async (req, res) => {
    const sock = getSock()
    if (!sock) return res.status(503).json({ ok: false, error: 'Bot não conectado' })
...
function createApp() {
  ...
  if (SERVE_FRONTEND) app.use('/', express.static(publicDir))

  const limiter = rateLimit()
  app.use('/api', cors, limiter)
  app.use('/api/v1', cors, limiter)
